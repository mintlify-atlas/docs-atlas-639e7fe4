...
This value is defined in `services/api.js` and can be overridden by setting the `VITE_API_URL` environment variable in your Vite project before the build. When running locally, ensure the backend server is started on port `3001` before launching the frontend.
